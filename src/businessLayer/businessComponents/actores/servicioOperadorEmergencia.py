...
class ServicioOperadorEmergencia:
    ...
    def crear(operador: OperadorEmergencia) -> Optional[OperadorEmergencia]:
        ...
        if operador.turno is None or not isinstance(operador.turno, str) or not operador.turno.strip():
            raise ValueError("El operador debe tener turno válido")
    ...
    def actualizar(id_operador: int, cambios: Dict[str, Any]) -> Optional[OperadorEmergencia]:
        ...
        if "turno" in cambios and isinstance(cambios["turno"], str):
            cambios["turno"] = cambios["turno"].strip()
