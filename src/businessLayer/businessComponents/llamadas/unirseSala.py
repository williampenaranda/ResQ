...
def generar_token_unirse_sala(nombre: str, nombre_sala: str) -> Tuple[str, str]:
    """
    Genera una identidad y un token JWT para un participante que se unirá a una sala existente de LiveKit.
    Similar a generar_token_participante pero específico para unirse a salas ya creadas.

    Args:
        nombre: Nombre a mostrar para el participante (usualmente nombre del operador).
    ...
    # Crear el token con permisos de unirse y audio (hablar/escuchar)
    token = (
